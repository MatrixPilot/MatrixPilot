...
#include "defines.h"
	...
	#if ( USE_SONAR == 1 )
		int sonar_rawaltitude ;      				// PWM converted sonar raw agl (above ground level) altitude in centimeters
		int sonar_altitude ; 		 				// cosine rmat8, tilt compensated sonar raw agl (above ground level) altitude
	...
	//  0- states.c (orig); 1- navigate.c; 2- altitudeCntrl.c and 3- libDCM.c
	#if (USE_BAROMETER == 1) && (ALTCAL_TRIG == 2) //   DEBUG trigger location
