...
void motorCntrl(void)
{
    ...
    else
    {
        ...
        // Compute the signals that are common to all 3 motors
        long_accum.WW = __builtin_mulus((unsigned int) pid_gains[ACCEL_K_INDEX], accelEarth[2]);
        accel_feedback = long_accum._.W1;
        motor_A = motor_B = motor_C = pwManual[THROTTLE_INPUT_CHANNEL] - accel_feedback;
...
#ifndef CONFIG_Y
#error ("You have not selected a configuration in options.h, select CONFIG_Y.")
#endif
