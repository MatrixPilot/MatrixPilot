...
// maps throttle input values of [2000, 2500)[2500, 3000)[3000, 3500)[3500, 4000]
// to a piecewise linear output range [v0, v1)[v1, v2)[v2, v3)[v3, v4]
struct fivePoint throttleMap;
struct fivePoint* pthrottleMap = &throttleMap;
unsigned int thr_x[5] = {2250,2625,3000,3375,3750};
unsigned int thr_y[5] = {2250,2850,3100,3250,(THROTTLE_LIMIT * 3750)};
unsigned int thr_yLow[5] = {2250,(.9*2850),(.9*3100),(.9*3250),(THROTTLE_LIMIT * 3750)};
...
void motorCntrl(void) {
    ...
    // limit throttle to 80% if battery is low
//#ifdef xyzzy
    if (!throttle_limit && (primary_voltage._.W1 < lowVoltageWarning)) {
        throttle_limit = true;
        fivePoint_init(pthrottleMap, thr_x, thr_yLow);
    } else if (throttle_limit && (primary_voltage._.W1 > (lowVoltageWarning + 500))) {
        // hysteresis of 500mV
        throttle_limit = false;
        fivePoint_init(pthrottleMap, thr_x, thr_y);
    }
//#endif
    if (motorsArmed < 2) {
        fivePoint_init(pthrottleMap, thr_x, thr_y);
    ...
    for (temp = 1; temp <= 4; temp++) {
        if (udb_flags._.radio_on) {
            if (temp == THROTTLE_INPUT_CHANNEL) {
                // apply throttle curve to throttle input
                pwManual[temp] = fivePointCurve(pthrottleMap, udb_pwIn[temp]);
