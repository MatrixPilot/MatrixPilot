...
{
 if (udb_heartbeat_counter == (10 * 400))
 {
 ...
 if (udb_heartbeat_counter <= ((unsigned int)(25 * (unsigned int)400)))
 {
  gps_startup_sequence( ((unsigned int)(25 * (unsigned int)400))-udb_heartbeat_counter ) ;
...
void udb_servo_callback_prepare_outputs(void)
{
 ...
 if ( udb_heartbeat_counter % (400/4) == 0 )
 {
  rxMagnetometer() ;
 }
...
void dcm_set_origin_location(long o_long, long o_lat, long o_alt)
{
 ...
 accum_nav.WW = __builtin_mulss( 305 , lat_origin._.W1 ) ;
...
struct relative3D dcm_absolute_to_relative(struct waypoint3D absolute)
{
 ...
 rel.y = (absolute.y - lat_origin.WW)/90 ;

 accum_nav.WW = ((absolute.x - long_origin.WW)/90) ;
