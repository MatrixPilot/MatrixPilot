...
int target_orientation[9] = { 0b0100000000000000 , 0 , 0 , 0 , 0b0100000000000000 , 0 , 0 , 0 , 0b0100000000000000 } ;

const int yaw_command_gain = ((long) 51 )*(0.03) ;
...
void motorCntrl(void)
{
 ...
 else if (abs(pwManual[1]-udb_pwTrim[1])< 100 )
 {

  motor_A = motor_B = motor_C = motor_D = pwManual[1] ;

  VectorCopy ( 9 , target_orientation , rmat ) ;

  commanded_roll = ( pwManual[2]
      - udb_pwTrim[2]) ;
  commanded_pitch = ( pwManual[3]
      - udb_pwTrim[3] ) ;
  commanded_yaw = ( pwManual[4]
      - udb_pwTrim[4] ) ;

                matrix_accum.x = rmat[4] ;
                matrix_accum.y = rmat[1] ;
                earth_yaw = rect_to_polar16(&matrix_accum) ;
  ...
  udb_pwOut[3] = udb_servo_pulsesat( motor_A ) ;
  udb_pwOut[4] = udb_servo_pulsesat( motor_B ) ;
  udb_pwOut[1] = udb_servo_pulsesat( motor_C ) ;
  udb_pwOut[2] = udb_servo_pulsesat( motor_D ) ;

 }
 else
 {
  commanded_roll = ( pwManual[2]
      - udb_pwTrim[2])*commanded_tilt_gain ;
  commanded_pitch = ( pwManual[3]
      - udb_pwTrim[3] )*commanded_tilt_gain ;
  commanded_yaw = ( pwManual[4]
      - udb_pwTrim[4] ) ;
  ...
  commanded_tilt[2] = 0b0100000000000000 ;
...
# 213 "../motorCntrl.c"
  ...
  min_throttle = udb_pwTrim[1] ;
  long_accum.WW = __builtin_mulus ( (unsigned int) (0b0100000000000000*1.0 ) , accelEarth[2] ) ;
  accel_feedback = long_accum._.W1 ;
  motor_A = motor_B = motor_C = motor_D = pwManual[1] - accel_feedback ;



  roll_error_integral.WW += ((__builtin_mulus ( (unsigned int ) (256.0*0b0100000000000000*0.05/((double) 400)), roll_error ))>>8) ;
  if ( roll_error_integral.WW > ((long int) 32768000 ) )
  {
   roll_error_integral.WW = ((long int) 32768000 ) ;
  }
  if ( roll_error_integral.WW < - ((long int) 32768000 ) )
  {
   roll_error_integral.WW = - ((long int) 32768000 ) ;
  ...
  pitch_error_integral.WW += ((__builtin_mulus ( (unsigned int ) (256.0*0b0100000000000000*0.05/((double) 400)), pitch_error ))>>8) ;
  if ( pitch_error_integral.WW > ((long int) 32768000 ) )
  {
   pitch_error_integral.WW = ((long int) 32768000 ) ;
  }
  if ( pitch_error_integral.WW < - ((long int) 32768000 ) )
  {
   pitch_error_integral.WW = - ((long int) 32768000 ) ;
  ...
  yaw_error_integral.WW += ((__builtin_mulus ( (unsigned int ) (256.0*0b0100000000000000*0.25/((double) 400)), yaw_error ))>>8) ;
  if ( yaw_error_integral.WW > ((long int) 32768000 ) )
  {
   yaw_error_integral.WW = ((long int) 32768000 ) ;
  }
  if ( yaw_error_integral.WW < - ((long int) 32768000 ) )
  {
   yaw_error_integral.WW = - ((long int) 32768000 ) ;
  ...
  long_accum.WW = __builtin_mulus ( (unsigned int) (0b0100000000000000*0.5) , yaw_error );
                yaw_rate_error = long_accum._.W1 - (theta[2] << 2);
  yaw_rate_error += yaw_error_integral._.W1 ;

  long_accum.WW = __builtin_mulus ( (unsigned int) (0b0100000000000000*2.0) , yaw_rate_error );
