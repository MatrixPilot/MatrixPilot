...
# 24 "../../libDCM/gpsParseCommon.c" 2


struct relative3D GPSlocation = { 0 , 0 , 0 } ;
struct relative3D GPSvelocity = { 0 , 0 , 0 } ;

union longbbbb lat_gps , long_gps , alt_sl_gps, tow ;
...
void udb_background_callback_triggered(void)
{
 ...
 if ( gps_nav_valid() )
 {
  ...
  accum_nav.WW = ( alt_sl_gps.WW - alt_origin.WW)/100 ;
  location[2] = accum_nav._.W0 ;


  accum.WW = __builtin_muluu ( 466 , cog_gps.BB ) + 0x00008000 ;
  ...
  rotate ( &location_deltaXY , cog_delta) ;
  ...
  air_speed_magnitudeXY = velocity_thru_air.x;
