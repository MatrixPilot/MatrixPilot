...
# 22 "../../libUDB/eeprom_udb4.c" 2
# 34 "../../libUDB/eeprom_udb4.c"
void udb_eeprom_init( void )
{
# 45 "../../libUDB/eeprom_udb4.c"
    I2C1BRG = 393 ;
...
unsigned char byte_in(unsigned char*, unsigned char);
void ACK_Poll(void);
...
unsigned int poll_set(volatile unsigned int* flag, int mask, unsigned int timeout) {
    while (((*flag & mask) == 0) && timeout > 0) {timeout--;}
    return timeout;
}


unsigned int poll_clr(volatile unsigned int* flag, int mask, unsigned int timeout) {
    while (((*flag & mask) != 0) && timeout > 0) {timeout--;}
    return timeout;
}
# 134 "../../libUDB/eeprom_udb4.c"
unsigned char byte_in(unsigned char* data, unsigned char ack)
{
    ...
    timeout = poll_set(&I2C1STAT, 0x02, 1000);
    if (timeout) {
        *data = I2CRCV;
        ...
        timeout = poll_clr(&I2C1CON, 0X10, 1000);
    }
    return (timeout > 0);
}
# 160 "../../libUDB/eeprom_udb4.c"
void ACK_Poll(void)
{
...
unsigned char eeprom_ByteRead(unsigned int address, unsigned char *data)
{
    ...
    if (status == 0) {
        ...
        status = byte_out((0xA1));
        status = byte_in(data, 1);
    }
    bstop();
    if (status)
        return 0;
