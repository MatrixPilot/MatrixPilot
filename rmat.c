...
// gfm calibrate_gyros is called by dcm_run_imu_step which is called at HEARTBEAT_HZ
        // consequently, 0.025 is replaced by 1/HEARTBEAT_HZ
		calib_accum = __builtin_mulsu(omegacorrPweighted[0], (uint16_t)(GGAIN/GYRO_CALIB_TAU/HEARTBEAT_HZ));
		gain_change = __builtin_divsd(calib_accum, spin_rate_over2);
		ggain[0] = adjust_gyro_gain(ggain[0], gain_change);

		calib_accum = __builtin_mulsu(omegacorrPweighted[1], (uint16_t)(GGAIN/GYRO_CALIB_TAU/HEARTBEAT_HZ));
		gain_change = __builtin_divsd(calib_accum, spin_rate_over2);
		ggain[1] = adjust_gyro_gain(ggain[1], gain_change);

		calib_accum = __builtin_mulsu(omegacorrPweighted[2], (uint16_t)(GGAIN/GYRO_CALIB_TAU/HEARTBEAT_HZ));
		gain_change = __builtin_divsd(calib_accum, spin_rate_over2);
