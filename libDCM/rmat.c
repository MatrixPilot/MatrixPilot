...
unsigned int accelerometer_samples = MAX_ACCEL_SAMPLES + 1;
...
void roll_pitch_drift() {
    ...
    boxcar(gplane, &filterState, gplaneFilt);

    // rotate acceleration vector into earth frame
    accelerometer_earth[0] = (VectorDotProduct(3, &rmat[0], gplaneFilt) << 1);
    accelerometer_earth[1] = (VectorDotProduct(3, &rmat[3], gplaneFilt) << 1);
    accelerometer_earth[2] = (VectorDotProduct(3, &rmat[6], gplaneFilt) << 1);
