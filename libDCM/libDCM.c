...
void udb_heartbeat_callback(void)
{
	...
	if ((USE_BAROMETER == 1 && ALTCAL_TRIG == 2) && (HILSIM != 1)&&(flags._.barometer_calibrated != 1)) //   DEBUG trigger location
	{	
...
void dcm_set_origin_location(int32_t o_lon, int32_t o_lat, int32_t o_alt)
{
	...
	flags._.origin_calibrated = 1;  // when confirmed, barometer alt calibration will fuse GPS altitude est. 
...
struct relative3D dcm_absolute_to_relative(struct waypoint3D absolute)  // TODO : explore for enhancing LOGO and adding relatives to WP in APM
{
