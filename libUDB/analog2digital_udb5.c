...
void __attribute__((__interrupt__, __no_auto_psv__)) _DMA0Interrupt(void)
{
    ...
    // VCC is the positive supply on the input PWM connectors: connected to AN0
    // through a 2:1 voltage divider
    // SVCC is the positive supply on the servo PWM connectors
    udb_vcc.input = CurBuffer[A_VCC_BUFF - 1];
    // 5V is the output of the 5V regulator: connected to AN1 through a 2:1 voltage divider
