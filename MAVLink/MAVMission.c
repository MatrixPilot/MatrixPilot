...
//uint8_t mavlink_system_mission_id = MAV_COMP_ID_MISSIONPLANNER;
...
//	if (target_component != mavlink_system.compid)
	if (target_component != MAV_COMP_ID_MISSIONPLANNER)
	{
...
static inline void MissionRequest(mavlink_message_t* handle_msg)
{
	...
	if (flightplan_index_get() == mavlink_waypoint_requested_sequence_number)
	{
 ...
 */
	...
	DPRINT("mission clear all: target_system %u, target_component %u\r\n", packet.target_system, packet.target_component);
	if (mavlink_check_target(packet.target_system, packet.target_component)) return;
...
//		mavlink_msg_mission_ack_send(MAVLINK_COMM_0, handle_msg->sysid, handle_msg->compid, type);
		mavlink_msg_mission_ack_send(MAVLINK_COMM_0, 55, 190, type);
	}
}
...
//	DPRINT("mission item\r\n");
	...
	if (!mavlink_flags.mavlink_receiving_waypoints)
	{
//		uint8_t type = 0; // ok (0), error(1)
//		DPRINT("flight plan received\r\n");
//		mavlink_msg_mission_ack_send(MAVLINK_COMM_0, handle_msg->sysid, handle_msg->compid, type);
//		DPRINT("MissionItem() mavlink_receiving_waypoints == 0\r\n");
		return;
	}
	// decode
	...
	{
		DPRINT("MissionItem() check target failed\r\n");
		return;
	}
...
//			wp = getWaypoint3D(mavlink_waypoint_requested_sequence_number);
			...
			//struct relWaypointDef current_waypoint = wp_to_relative(waypoints[flightplan_index_get()]);
