...
/* ************  BAROMETER SUPPORT External Variables ************ */
	...
	extern inline int32_t  get_barometer_pressure(void);
	extern inline int32_t  get_barometer_gndaltitude(void);
	extern inline int32_t  get_barometer_aglaltitude(void);
	extern inline int32_t  get_barometer_aslaltitude(void);

	extern int16_t barometer_temperature;	
	extern int32_t barometer_pressure; 
	extern int32_t barometer_gndaltitude; 
	extern int32_t barometer_aglaltitude;
	extern int32_t barometer_aslaltitude;  //void calBarometer(void) 
...
extern int16_t tofinish_line;  // >>>>>>>>>>> uncoment to fix build error
//extern int16_t progress_to_goal; // Fraction of the way to the goal in the range 0-4096 (2^12)
extern int8_t desired_dir;  // >>>>>>>>>>> uncoment to fix build error
