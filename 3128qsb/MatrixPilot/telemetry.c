...
//			if (udb_heartbeat_counter % (HEARTBEAT_HZ/4) != 0)  // Every 2 runs (5 heartbeat counts per 8Hz)
			...
			else
			{
				...
				serial_output("imx%i:imy%i:imz%i:lex%i:ley%i:lez%i:fgs%X:ofc%i:tx%i:ty%i:tz%i:G%d,%d,%d:",IMUlocationx._.W1,IMUlocationy._.W1,IMUlocationz._.W1,
					...
					#if  (USE_SONAR == 1)
						//serial_output("sr%i,sa%i,cp%i:", sonar_distance, sonar_height_to_ground, cos_pitch_roll );
					...
					//#if (USE_BAROMETER == 1) 
						...
						barometer_gndaltitude = get_barometer_gndaltitude();
						barometer_aglaltitude = get_barometer_aglaltitude();
						barometer_aslaltitude = get_barometer_aslaltitude();
						serial_output("t%i,p%i,ga%i,ag%i,as%i:",
						(int)barometer_temperature, (long)barometer_pressure, (long)barometer_gndaltitude,
						(long)barometer_aglaltitude,(long)barometer_aslaltitude);
...
/*  ***   Support for Barometer debug serial output ***  */
	...
	void serial_output_8hz( void )
	{
		barometer_temperature = get_barometer_temperature();
		barometer_pressure = get_barometer_pressure(); 
		barometer_gnd_altitude = get_barometer_gndaltitude();
		barometer_agl_altitude = get_barometer_aglaltitude();
		barometer_asl_altitude = get_barometer_aslaltitude();
		serial_output("t%i,p%i,ga%i,ag%i,as%i:\r\n",
		(int16_t)barometer_temperature, (int32_t)barometer_pressure, (int32_t)barometer_gndaltitude,
		(int32_t)barometer_aglaltitude, (int32_t)barometer_aslaltitude );
		return ;
	}
