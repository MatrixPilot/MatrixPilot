...
static void setup_origin(void)
{
	...
	else
	{
		...
		// ROUGHED-IN TODO: IMPLEMENT UNDER A SONAR HEALTH CHECK, WITHIN
...
void dcm_callback_gps_location_updated(void)
{
	if (flags._.save_origin)
	{
		...
		#if ((USE_BAROMETER == 1 && ALTCAL_TRIG == 1) && (HILSIM != 1)) //   DEBUG trigger location
			BarometerCalOne();  				// first barometer calibration (barometerCntrl.c)
		#endif
		//  0- states.c and libDCM.c (def); 1- navigate.c; 
		#if ((USE_BAROMETER == 1 && ALTCAL_TRIG == 1) && (HILSIM != 1)) //   DEBUG trigger location
			BarometerCalTwo();  				// second barometer calibration (barometerCntrl.c)
		#endif
	}
