	...
	void runI2CQueuedSensors(void) // USE QUEUED I2C DRIVER at 40Hz
	{
		...
		if ( udb_heartbeat_counter % 10 == 0 )  // def 10 for 4hz, 4 for 10hz and 2 for 20hz
		{
			...
			// include barometer calibration flag
				...
				rxBarometer(udb_barometer_callback); 		// RUN BAROMETER FUNCTION~ in barometerCntrl.c udb_barometer_callback
			#endif
		}
	}
#else
...
void udb_heartbeat_callback(void)
{
	#if (I2C2_QUEUED_ON  == 1)
		runI2CQueuedSensors();
		// TESTING runtime trigger location: 0- barometerCntrl.c (def); 1- gpsParseCommon.c; 2- libDCM.c; 
		#if ( ALTRUN_TRIG == 2) 
			if (udb_heartbeat_counter % (HEARTBEAT_HZ / 4) == 0)  			// 4HZ
			{
			...
			if (udb_heartbeat_counter % (HEARTBEAT_HZ / 8) == 0)  				// 8HZ
			{
...
void dcm_calibrate(void)
{
	...
	if (dcm_flags._.calib_finished)
	{
		...
		//  0- states.c and libDCM.c (def); 1- navigate.c;
		#if ((USE_BAROMETER == 1 && ALTCAL_TRIG == 0) && (HILSIM != 1)) //   DEBUG trigger location
			barometerCalTwo();  				// first barometer calibration (barometerCntrl.c), 
		#endif
