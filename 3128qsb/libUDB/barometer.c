...
// Barometer oversampling setting [OSS] can be set from 0 thru 3
//				  [ms]  Ave/cur/uA   [hPA]      [m]
...
// #define OSS 								1  // moved over to options.h
...
#include "barometer.h"
	...
	typedef union 
	{
	...
	#if (I2C2_QUEUED_ON == 0) 					//  >>>>>>>>>>>>    I2C2_QUEUED_ON (options.h)  
		...
		{	
			if (I2CtrxOK == true)
			{
				...
				// TEST_WITH_DATASHEET_VALUES option
			}
		}
		
		// Calculate temperature given ut.
		// Value returned will be in units of 0.1 deg C
		static int32_t bmp085CalcTemperature(unsigned int ut)
		{
			long x1, x2;
		...
		static long bmp085CalcPressure(unsigned long up)
		{
			//  CHANGED, ORIG 333 BELOW
			//long x1, x2, x3, b3, b6, p;
			//unsigned long b4, b7;      
			long x1, x2, x3, b3, b6, b7, p;
			unsigned long b4; 

			//  CHANGED, ORIG 222 BELOW
				b6 = b5 - 4000; 
			// Calculate B3 
		...
		void ReadBarPres_callback(boolean I2CtrxOK)
		{
			...
			if( I2CtrxOK == true )
			{
				...
				// TEST_WITH_DATASHEET_VALUES option holder here
	...
	#endif   					//  >>>>>>>>>>>>    I2C2_QUEUED_ON (options.h)  
	...
	/*
			...
			// TEST_WITH_DATASHEET_VALUES option
				#ifdef TEST_WITH_DATASHEET_VALUES   ////  TEMP TEST VALUES (options.h): REMOVE AFTER TESTING  ////
					bc.ac1 = 408;
					bc.ac2 = -72;
					bc.ac3 = -14383;
					bc.ac4 = 32741;
