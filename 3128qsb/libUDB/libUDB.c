...
#include "osd.h"
#include "I2C.h"
/*#if ((USE_I2C1_DRIVER == 1) || (USE_I2C2_DRIVER == 1))
	#include "I2C.h"
#endif
*/
...
void udb_init(void)
{
	...
	#endif
	udb_init_clock();
	udb_init_capture();         // provides for PWM capture function in radioIn.c 

	#if (USE_SONAR == 1)  		// provides for PWM sonar function in radioIn.c 
	    udb_init_sonar();
	#endif
	#if (CONSOLE_UART != 1)
		udb_init_GPS();
