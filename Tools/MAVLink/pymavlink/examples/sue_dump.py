...
# This program is licensed for use with GPL V3 or later.
'''
...
# The following alternative example will read a raw MatrixPilot MAVLink Log, and output
# SERIAL_UDB_EXTRA formatted strings. Note that a raw log, is one in which timestamps have
# not been inserted on the receiving end at the ground station. QGroundControl will insert timestamps.
# In this example, the Original OpenLog File is called,  flight.log.raw

# Create a file handle using the normal standard output channel.
f = sys.stdout
# Alternatively, you can write the standard output to f file ...
#f = open("flight_sue.txt", 'w')
# Note that writing to a file is preferred because errors are then written to standard out, and only
# well formatted SERIAL_UDB_EXTRA messages will end up in the telemetry file.

#master = mavutil.mavlink_connection('mavproxy logs xxxl flight2 01-05-12/flight.log.raw', notimestamps = True )
master = mavutil.mavlink_connection('flight.log.raw', notimestamps = True )
