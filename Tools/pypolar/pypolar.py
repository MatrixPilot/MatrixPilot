...
class polar_parser(object):
    ...
    def process(self, bodycd, alpha_step, MAC ):
        ...
        for plr in self.polars:
            ...
            plr.gen_polar_name()
            if(len(plr.points) > self.max_points_count ):
                self.max_points_count = len(plr.points)
        self.Re_list.sort()
        self.aspd_list.sort()
        self.flap_list.sort()
    ...
    def export_to_c(self, outdirectory):
        
        if(outdirectory != ""):
            op_directory = outdirectory
        else:
            op_directory = self.directory

        filepath = os.path.join(op_directory, "polar_data.c")
        ...
        for plr in self.polars:
            ...
            f.write(text)
            for pnt in plr.points:
                text = ("{:d} , {:d} , {:d} , {:d}").format(self.ftoQ16(pnt.alpha), self.ftoQ16(pnt.Cl), self.ftoQ16(pnt.Cd + self.bodyCd), self.ftoQ16(pnt.Cm) )
                f.write("{ " + text + " },\n")
            f.write("};\n")
            f.write("\n")
            f.write("\n")
...
#        f.write("const unsigned int afrm_aspd_count = " + str(len(self.aspd_list)) + ";\n")
#        f.write("const unsigned int afrm_flap_count = " + str(len(self.flap_list)) + ";\n")
#        f.write("const unsigned int afrm_polar_count = " + str(len(self.flap_list) * len(self.aspd_list)) + ";\n\n")
        
        f.write("const _Q16 afrm_polar_flap_settings[] = { ")
        for flap in self.flap_list:
            f.write( str(self.ftoQ16(flap)) )
            f.write(" , ")
        f.write("};\n\n")          
                
        f.write("const _Q16 afrm_polar_aspd_settings[] = { ")
        for aspd in self.aspd_list:
            f.write( str(self.aspd_to_cm(aspd)) )
            f.write(" , ")
        f.write("};\n\n")          
                
        f.close()
        
        filepath = os.path.join(op_directory, "polar_data.h")
        f = open(filepath, "w")
        f.write("// File automatically generated by pypolar - DO NOT EDIT\n")
        f.write("//\n\n")
        f.write("#define AFRM_MAX_POLAR_POINTS " + str(self.max_points_count) + "\n\n")
        f.write("#define AFRM_ASPD_POINTS " + str(len(self.aspd_list)) + "\n\n")
        f.write("#define AFRM_FLAP_POINTS " + str(len(self.flap_list)) + "\n\n")
        f.write("#define AFRM_POLAR_COUNT " + str(len(self.flap_list) * len(self.aspd_list)) + "\n\n")
...
if __name__ == '__main__':
    ...
    parser.add_option("--outdirFBW", dest="outdirFBW", help="Output directory is relative path to MatrixPilotFBW", action='store_true', default=False)
    ...
    if(opts.outdirFBW):
        outdir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", "..","MatrixPilotFBW")
    else:
        outdir = ""
    pparser.export_to_c(outdir)
