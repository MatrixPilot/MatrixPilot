...
int JSBSim_real_init(int argc, char* argv[]);
...
int JSBSim_init(int argc, char* argv[])
{
  ...
  try {
//    real_main(argc, argv);
    result = JSBSim_real_init(argc, argv);
  } catch (string& msg) {
...
#ifdef ID_JSBBASE // JSBSim version from GitHub Inspirati/JSBSim
...
#else // JSBSim version from GitHub JSBSim-Team/jsbsim
...
bool options(int count, char **arg)
{
  ...
  for (i=1; i<count; i++) {
    ...
    if (keyword == "--help") {
      ...
      cout << endl << "  JSBSim Version: " << FDMExec->GetVersion() << " (MatrixPilot)" << endl << endl;
      exit (0);
