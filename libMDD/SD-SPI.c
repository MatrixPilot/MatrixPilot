...
void UARTSendLineFeedCarriageReturn(void)
{
	printf("\r\n");
	delay_ms(100);
}
...
BYTE MDD_SDSPI_MediaDetect (void)
{
	...
	if (SPIENABLE == 0)
	{
		...
		printf("MediaDetect - calling InitSPISlowMode()\r\n");
		InitSPISlowMode();
		printf("MediaDetect - InitSPISlowMode() complete.\r\n");
	    ...
	    {
			printf("MediaDetect - invalid R1 response.\r\n");
	        CloseSPIM();
	        return FALSE;
	    }    
	    else
	    {
			printf("MediaDetect - Card is presumably present.\r\n");
    		...
    		{
				printf("MediaDetect - Card was initialized correctly.\r\n");
    			/* if the card was initialized correctly, it means it is present */
    			return TRUE;
    		}
    		else 
    		{
				printf("MediaDetect - Media ERROR.\r\n");
        		CloseSPIM();
    			return FALSE;
    		}		
	...
	{
		printf("MediaDetect - The SPI module was already enabled.\r\n");
...
//	InitUART();
    ...
    {
        #ifdef __DEBUG_UART  
        printf("Media failed CMD0 too many times. R1 response byte = ");
        PrintRAMBytesUART(((unsigned char*)&response + 1), 1);
        UARTSendLineFeedCarriageReturn();
        printf("Trying CMD12 to recover.\r\n");
        #endif
    ...
    if(((response.r7.bytewise.argument._returnVal & 0xFFF) == 0x1AA) && (!response.r7.bitwise.bits.ILLEGAL_CMD))
   	{
        ...
        #ifdef __DEBUG_UART  
        printf("Media successfully processed CMD8. Response = ");
		...
		{				
			...
			{
    		    #ifdef __DEBUG_UART  
                printf("Media successfully processed CMD55/ACMD41 and is no longer busy.\r\n");
				#endif
		...
		if(response.r7.bytewise.argument._returnVal & 0x40000000)    //Note the HCS bit is only valid when the busy bit is also set (indicating device ready).
		{
		    ...
		    #ifdef __DEBUG_UART  
            printf("Media successfully processed CMD58: SD card is SDHC v2.0 (or later) physical spec type.\r\n");
            #endif
        }				
        else
        {
            ...
            #ifdef __DEBUG_UART  
            printf("Media successfully processed CMD58: SD card is standard capacity v2.0 (or later) spec.\r\n");
            #endif
        } 
    ...
    else
	{
        ...
        #ifdef __DEBUG_UART  
        printf("CMD8 Unsupported: Media is most likely MMC or SD 1.x device.\r\n");
        #endif
        ...
        {
            #ifdef __DEBUG_UART  
            printf("CMD1 Successfully processed, media is no longer busy.\r\n");
            #endif
    ...
    {
        #ifdef __DEBUG_UART  
        printf("CMD9 Successfully processed: Read CSD register.\r\n");
        printf("CMD9 response R1 byte = ");
    ...
    #ifdef __DEBUG_UART  
    printf("CSD data structure contains: ");
    ...
    #ifdef __DEBUG_UART  
    printf("Returning from MediaInitialize() function.\r\n");
    #endif
