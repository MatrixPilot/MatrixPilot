...
int16_t sonar_aglaltitude = 0;
int16_t sonar_rawaglaltitude = 0;
uint32_t cos_sonarproll = 0;

inline int16_t get_sonar_rawaglaltitude(void) {
    return sonar_rawaglaltitude;
}
...
inline uint32_t get_cos_sonarproll(void) {
    return cos_sonarproll;
}
...
void calSonarAGLAltitude(void) {
    if (udb_flags._.sonar_updated == 1) {
        ...
        if (sonar_rawaglaltitude > EFFECTV_SONAR_ALTRANGE) {
            ...
            if (valReadIndex > SONAR_VALREADS) {
                valReadIndex = SONAR_VALREADS;
                ...
                sonar_aglaltitude = SONAR_VALREADS; //MAXIMUM_SONAR_ALTRANGE;
            }
        }
