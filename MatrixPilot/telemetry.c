			...
			else
			{
				...
				serial_output("imx%i:imy%i:imz%i:lex%i:ley%i:lez%i:fgs%X:ofc%i:tx%i:ty%i:tz%i:G%d,%d,%d:",IMUlocationx._.W1,IMUlocationy._.W1,IMUlocationz._.W1,
					...
					/* ***  sonar and barometer feed ***  */
						...
						(int16_t)get_barometer_temperature(),(float)get_barometer_pressure(),(int32_t)get_barometer_aslaltitude());
					#endif
					/* ***  sonar and barometer feed end ***  */
...
void serial_output_8hz( void )
{
	serial_output("ot%i,op%i,oa%i,rt%i,rp%i,at%i,ap%i,aa%i:\r\n",
	(int16_t)get_barometer_temperatureorgn(), (float)get_barometer_pressureorgn(),(int32_t)get_barometer_altitudeorgn()  
	(int16_t)get_barometer_temperature(),(float)get_barometer_pressure(),
	(int16_t)get_barometer_rtavetemperature(),(float)get_barometer_rtavepressure(), 
	(int32_t)get_barometer_aslaltitude());
	return ;
}
