...
#define SERIAL_BAUDRATE 57600   //  Rem. to adjust serial telemetry log to this baud rate to support extra data from Sonar
#else
#define SERIAL_BAUDRATE 19200 // default
#endif
#ifdef SETUP_WARNINGS_ON
...
void serial_output_8hz(void) {
    ...
    switch (telemetry_counter) {
        ...
        case 8:
            serial_output("\r\nF4:APSVER=");
            serial_output(APSVERSION);
            serial_output(":\r\n");
            serial_output("F5:BOARD=%i:AIRFRAME=%i:GPS=%i:RCON=0x%X:TRAP_FLAGS=0x%X:TRAP_SOURCE=0x%lX:ALARMS=%i:"
                    "CLOCK=%i:FP=%d:\r\n",
                    BOARD_TYPE, AIRFRAME_TYPE, GPS_TYPE, get_reset_flags(), trap_flags, trap_source, osc_fail_count, CLOCK_CONFIG, FLIGHT_PLAN_TYPE);
            break;
        case 7:
#if  (USE_BAROMETER == 1)
            ...
            serial_output(":\r\n");
#if  (USE_SONAR == 1)
            serial_output("F7:USESNR=%i:USESNRALT=%i:ALTRANGE=%i:MAXALTR=%i:MINVALR=%i:",
                    USE_SONAR, SONAR_ALTITUDE, EFFECTV_SONAR_ALTRANGE, MAXIMUM_SONAR_ALTRANGE, SONAR_VALREADS);
#else
            serial_output("F7:USESNR=");
            serial_output(USE_SONAR);
#endif
            serial_output(":\r\n");
            break;
        case 6:
            serial_output("F8:IDA=");
            serial_output(ID_VEHICLE_MODEL_NAME);
            serial_output(":IDB=");
            serial_output(ID_VEHICLE_REGISTRATION);
            serial_output(":\r\n");
            serial_output("F9:IDC=");
            serial_output(ID_LEAD_PILOT);
            serial_output(":IDD=");
            serial_output(ID_URL);
            serial_output(":\r\n");
            break;
        case 5:
            serial_output("F10:R_STAB_A=%i:R_STAB_RD=%i:P_STAB=%i:Y_STAB_R=%i:Y_STAB_A=%i:AIL_NAV=%i:RUD_NAV=%i:AH_STAB=%i:AH_WP=%i:RACE=%i:\r\n",
            ...
            break;
        case 4:
            serial_output("F11:YAWKP_A=%5.3f:YAWKD_A=%5.3f:ROLLKP=%5.3f:ROLLKD=%5.3f:A_BOOST=%3.1f:\r\n",
                    YAWKP_AILERON, YAWKD_AILERON, ROLLKP, ROLLKD, AILERON_BOOST);
            break;
        case 3:
            serial_output("F12:P_GAIN=%5.3f:P_KD=%5.3f:RUD_E_MIX=%5.3f:ROL_E_MIX=%5.3f:E_BOOST=%3.1f:\r\n",
                    PITCHGAIN, PITCHKD, RUDDER_ELEV_MIX, ROLL_ELEV_MIX, ELEVATOR_BOOST);
            break;
        case 2:
            serial_output("F13:Y_KP_R=%5.4f:Y_KD_R=%5.3f:RLKP_RUD=%5.3f:RLKD_RUD=%5.3f:RUD_BOOST=%5.3f:RTL_PITCH_DN=%5.3f:\r\n",
                    YAWKP_RUDDER, YAWKD_RUDDER, ROLLKP_RUDDER, ROLLKD_RUDDER, RUDDER_BOOST, RTL_PITCH_DOWN);
            break;
        case 1:
            serial_output("F14:H_MAX=%6.1f:H_MIN=%6.1f:MIN_THR=%3.2f:MAX_THR=%3.2f:PITCH_MIN_THR=%4.1f:PITCH_MAX_THR=%4.1f:PITCH_ZERO_THR=%4.1f:\r\n",
...
#define SONAR_NOINPUT_VALUE					999999 // Distance denotes that no sonar reading was returned from sonar device
extern uint16_t udb_pwm_sonar;
