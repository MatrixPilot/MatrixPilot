...
unsigned int16_t data_service_state = DATA_SERVICE_STATE_NOT_STARTED;

// Handle to event that will trigger the low priority service routine
unsigned int16_t data_service_event_handle = INVALID_HANDLE;
...
unsigned int16_t serialise_items_to_buffer(uint16_t table_index);

// Serialise the buffer to a list of data items/variables
// returns size of items serialised
unsigned int16_t serialise_buffer_to_items(uint16_t table_index);
...
unsigned int16_t data_services_get_table_index(uint16_t data_storage_handle);

// Calculate the size of the complete list of variable items in bytes
unsigned int16_t data_services_calc_item_size(uint16_t table_index);

// Tracking index into table
unsigned int16_t data_services_table_index = 0;

// Flags that determine how and when to do serialisation
unsigned int16_t data_services_serialize_flags = 0;
