...
static boolean udb_throttle_enable = false;
static boolean writeGains = false;
...
static void storeGain(int index) // unused
{
...
static void adjust_gain(int index, int delta)
{
    if (delta > 0)
    {
        delta = RMAX * GAIN_INC;
...
static int gainAdjIndex[] = { ADJ_GAIN_0, ADJ_GAIN_1, ADJ_GAIN_2 };

static void check_gain_adjust(void)
{
...
#endif // ENABLE_GAINADJ

void quad_background_task(void) // this was called run_background_task()
{
...
//    Idle(); // this now called from udb_run()
}
