...
static int16_t logo_value_for_identifier(uint8_t ident) {
    ...
    switch (ident) {
        ...
        {
            return -999; //  return dummy value to trap absence of sonar sensor device in LOGO
        }
#endif
    ...
    switch (instr.cmd) {
        ...
        case 3: // Forward/Back
            switch (instr.subcmd) {
                case 0: // Forward
                {
                    ...
                    cangle = turtleAngles[currentTurtle]; // 0-359 (clockwise, 0=North)
        ...
        case 4: // Rotate
            switch (instr.subcmd) {
                ...
                {
                    int16_t angle;
                    angle = turtleAngles[currentTurtle] + instr.arg;
        ...
        case 9: // Modify PARAM
            switch (instr.subcmd) {
                ...
                {
                    int16_t ind;
                    ind = get_current_stack_parameter_frame_index();
        ...
        case 19:
        {
            ...
            val = logo_value_for_identifier(instr.subcmd);
            condTrue = false;
...
static void process_instructions(void) {
    ...
    while (1) {
        boolean do_fly;
        do_fly = process_one_instruction(currentInstructionSet[instructionIndex]);
